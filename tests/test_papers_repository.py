...
def test_insert_then_find_by_id(test_db):
    papers_repository.insert(
        paper_id="p1", subject="Mathematics", class_name="Grade 1",
        total_marks=42, question_ids=["q1", "q2", "q3"],
    )
...
def test_insert_with_null_class_name(test_db):
    papers_repository.insert(
        paper_id="p1", subject="English", class_name=None,
        total_marks=10, question_ids=[],
    )
    paper = papers_repository.find_by_id("p1")
